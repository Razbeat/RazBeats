...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose, items, onRemoveItem, onCheckout }) => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className={`fixed inset-y-0 left-0 w-full md:w-[450px] bg-[#F5F2EB] z-[70] shadow-2xl transform transition-transform duration-500 ease-in-out border-r border-[#D6D1C7] flex flex-col ${
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#D6D1C7]">
          <h2 className="text-xl font-serif text-[#2C2A26]">סל הקניות שלך ({items.length})</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-[#5D5A53]">סל הקניות שלך ריק.</p>
            </div>
          ) : (
            items.map((item, idx) => (
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          <div className="flex justify-between items-center mb-6">
            <span className="text-sm font-medium uppercase tracking-widest text-[#5D5A53]">סה״כ</span>
            <span className="text-xl font-serif text-[#2C2A26]">₪{total}</span>
          </div>
          <p className="text-xs text-[#A8A29E] mb-6 text-center">משלוח ומיסים יחושבו בקופה.</p>
          <button 
            onClick={onCheckout}
